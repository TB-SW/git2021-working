...
const Todo = () => {
  ...
  const del = (id: number) => {
    ...
    // 주로 filter 함수를 사용
    // filter 함수로 해딩 id를 제외하고 새로운 배열로 리턴함
  ...
  return (
    <>
      ...
      <ul id="ul-list" className="list-group list-group-flush mt-3">
        {todoList.length == 0 && (
        ...
        {todoList.map((item) => (
          <li className="list-group-item d-flex" key={(item.id)}>
            ...
            {!item.isEdit && (
              <button
                className="btn btn-outline-secondary btn-sm ms-2 me-1 float-end text-nowrap"
                onClick={() => {
                  edit(item.id, true);
                }}
              >
